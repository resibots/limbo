    ...
    const char* input_parameter_filename)
{
  ...
  ERRORMESSAGE("cmaes_init called twice, which will lead to a memory leak, use
cmaes_exit first",0,0,0);
  printf("Warning: cmaes_init called twice, which will lead to a memory leak,
use cmaes_exit first\n");
}*/

    t->version = version;
    /* assign_string(&t->signalsFilename, "cmaes_signals.par"); */

    readpara_init(&t->sp, dimension, inseed, inxstart, inrgstddev, lambda,
...
static void Adapt_C2(cmaes_t* t, int hsig)
{
    ...
    if (t->sp.ccov != 0. && t->flgIniphase == 0)
    {
        ...
        double ccovmu = douMin(t->sp.ccov * (1 - 1. / t->sp.mucov) * (flgdiag ? (N + 1.5) / 3. : 1.),
            1. - ccov1);
...
const char* cmaes_TestForTermination(cmaes_t* t)
{
    ...
    if ((t->gen > 1 || t->state > 1) && t->sp.stStopFitness.flg && t->rgFuncValue[t->index[0]] <= t->sp.stStopFitness.val)
    ...
    range = douMax(rgdouMax(t->arFuncValueHist,
                ...
                rgdouMax(t->rgFuncValue, t->sp.lambda)) - douMin(rgdouMin(t->arFuncValueHist,
                                                                     (int)douMin(t->gen, *(t->arFuncValueHist - 1))),
